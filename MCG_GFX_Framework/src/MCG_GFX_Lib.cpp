...
bool MCG::Init( glm::ivec2 windowSize )
{
	...
	int size = _winSize.x * _winSize.y * 3;
...
{
	//Storing pixel colour data
	datas[position.y * _winSize.x * 3 + position.x * 3] = (unsigned char)colour.x;
	datas[(position.y * _winSize.x * 3 + position.x * 3) + 1] = (unsigned char)colour.y;
	datas[(position.y * _winSize.x * 3 + position.x * 3) + 2] = (unsigned char)colour.z;
}
...
SDL_Surface* MCG::getSurface()
{
	return frameBuffer;
}

SDL_Texture* MCG::getTexture()
{
	return _texture;
}

void MCG::createFrameBuffer()
{
	...
	int depth, pitch;
	depth = 24;
	pitch = 3 * _winSize.x; // 3 bytes per pixel * pixels per row

	frameBuffer = SDL_CreateRGBSurfaceFrom((void*)datas, _winSize.x, _winSize.y, depth, pitch, rmask, gmask, bmask, amask);
}

void MCG::createTexture()
{
	_texture = SDL_CreateTextureFromSurface(_renderer, frameBuffer);
}